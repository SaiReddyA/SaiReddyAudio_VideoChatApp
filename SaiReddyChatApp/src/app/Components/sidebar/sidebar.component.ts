...
export class SidebarComponent implements OnInit {
    ...
    selectedItem: string = 'person'; // Input to receive the selected item from the Header component
    activeUsers: UserInfo[] = [];
    typingUser: string | null = null;
    userName: string = '';
    currentUserId:string = "";
    ngOnInit() {
      this.chatService.selectedItem$.subscribe(item => {
  ...
  selectUser(user: UserInfo) {
    ...
    this.chatService.markMessagesAsRead(this.chatService.getCurrentUserId(), user.userId);
  }
